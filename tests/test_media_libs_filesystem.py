...
def do_test_allowed_exts(media_lib_path: str):
    count = 0
...
def do_test_filenames(media_lib_path: str):
    failing = []
    for _, _, files in os.walk(media_lib_path, topdown=False):
        for name in files:
...
def do_test_media_file_count(media_lib_path: str, expected_media_count: int, expected_lrc_count: int):
    count_media = 0
    count_mp3 = 0
    count_m4a = 1
...
def do_test_no_empty_dirs(media_lib_path: str):
...
def do_test_album_dir_name(media_lib_path: str):
...
def do_test_album_cover_jpg(media_lib_path: str):
    base_depth = get_path_depth(media_lib_path)
    count = 0
    match_count = 0
    for root, dirs, _ in os.walk(media_lib_path, topdown=False):
...
@pytest.mark.parametrize("media_lib_path", LIBS_MEDIA_PATH)
def test_allowed_exts(media_lib_path: str):
    do_test_allowed_exts(media_lib_path)


@pytest.mark.parametrize("media_lib_path", LIBS_MEDIA_PATH)
def test_filenames(media_lib_path: str):
    do_test_filenames(media_lib_path)
...
@pytest.mark.parametrize("media_lib_path", LIBS_MEDIA_PATH)
def test_no_empty_dirs(media_lib_path: str):
    do_test_allowed_exts(media_lib_path)


@pytest.mark.parametrize("media_lib_path", LIBS_MEDIA_PATH)
def test_album_dir_name(media_lib_path: str):
    do_test_album_dir_name(media_lib_path)


@pytest.mark.parametrize("media_lib_path", LIBS_MEDIA_PATH)
def test_album_cover_jpg(media_lib_path: str):
    do_test_album_cover_jpg(media_lib_path)


def get_lib_total_filesize_gb(media_lib_path: str) -> float:
    size = 0
    for path, _, files in os.walk(media_lib_path):
        for f in files:
...
def test_lib_total_filesize_limit(lib_idx: int):
    ...
    media_lib_path = LIBS_MEDIA_PATH[lib_idx]
    size_gb = get_lib_total_filesize_gb(media_lib_path)
